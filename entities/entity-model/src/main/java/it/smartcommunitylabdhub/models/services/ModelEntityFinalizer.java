...
@Component
@Slf4j
public class ModelEntityFinalizer implements EntityFinalizer<Model>, InitializingBean {
    ...
    @Override
    public void finalize(@NotNull Model model) throws StoreException {
        log.debug("finalize model with id {}", model.getId());

        String id = model.getId();

        if (filesService != null && filesInfoService != null) {
            //files
            log.debug("cascade delete files for model with id {}", id);
