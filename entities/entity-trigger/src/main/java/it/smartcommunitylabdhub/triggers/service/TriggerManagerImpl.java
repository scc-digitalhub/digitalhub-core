...
package it.smartcommunitylabdhub.triggers.service;
...
import it.smartcommunitylabdhub.triggers.TriggerManager;
...
public class TriggerManagerImpl implements TriggerManager {
    ...
    @Override
    public List<Trigger> listTriggersByTask(@NotNull String project, @NotNull String task) {
        log.debug("list triggers for task {}", task);
        try {
            //we expect task to be a valid path
            // (task.getKind() + "://" + task.getProject() + "/" + task.getId());
            String path = task;
