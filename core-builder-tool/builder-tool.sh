...
process_context_ref() {
    ...
    case "$protocol" in
        "git+https")
            ...
            clone_url="$repo"
            [ -n "$username" ] && clone_url="https://$username:$password@$repo"
            temp_suffix=$(echo -n "$git_uri" | base64 | tr -d '=' | tr '/+' '_-' | cut -c1-32)
            ...
            [ -n "$branch" ] && git clone --branch "$branch" --config core.askpass='' "$clone_url" "$temp_clone_dir" || git clone --config core.askpass='' "$clone_url" "$temp_clone_dir"
