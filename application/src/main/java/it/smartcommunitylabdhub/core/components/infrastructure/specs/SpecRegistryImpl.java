...
public class SpecRegistryImpl implements SpecRegistry, SpecValidator, InitializingBean {
	 ...
	 * @return An instance of the specified spec type, or null if not found or in case of
	 * errors.
	 */
	@Override
			...
			if (binder.getBindingResult().hasErrors()) {
				try {
					MethodParameter methodParameter = new MethodParameter(
							this.getClass().getMethod("validateSpec", Spec.class), 0);
					MethodArgumentNotValidException ex = new MethodArgumentNotValidException(methodParameter, binder.getBindingResult());
	...
	private record SpecRegistration(SpecType type, Class<? extends Spec> spec, SpecFactory<? extends Spec> factory,
			Schema schema) {
	}
