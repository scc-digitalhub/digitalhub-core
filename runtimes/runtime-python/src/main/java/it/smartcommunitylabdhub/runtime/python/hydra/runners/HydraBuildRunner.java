...
import it.smartcommunitylabdhub.runtime.python.build.PythonBaseBuildRunner;
...
@Slf4j
public class HydraBuildRunner extends PythonBaseBuildRunner {

    public static final int MIN_IMAGE_NAME_LENGTH = 3;

    public HydraBuildRunner(PythonProperties properties, K8sBuilderHelper k8sBuilderHelper) {
    ...
    public K8sContainerBuilderRunnable produce(Run run, Map<String, String> secretData) {
        HydraBuildRunSpec runSpec = new HydraBuildRunSpec(run.getSpec());
        HydraBuildTaskSpec taskSpec = runSpec.getTaskBuildSpec();
        ...
        List<ContextRef> contextRefs = new ArrayList<>(PythonRunnerHelper.createContextRefs(sourceCode));
        ...
        return K8sContainerBuilderRunnable
            ...
            .task(OpeninferenceBuildTaskSpec.KIND)
            ...
            .resources(
                k8sBuilderHelper != null
                    ? k8sBuilderHelper.convertResources(runSpec.getTaskBuildSpec().getResources())
                    : null
            )
