...
public class HydraRunnerHelper {
     ...
     * @return
     */
    protected static List<CoreVolume> createVolumes(
        Run run,
        K8sFunctionTaskBaseSpec taskSpec,
        String volumeSizeSpec,
        K8sBuilderHelper k8sBuilderHelper,
        String id,
        CoreVolume.VolumeType volumeType
    ) {
        ...
        Optional.ofNullable(k8sBuilderHelper).ifPresent(helper -> {
            Optional.ofNullable(buildSharedSharedVolume(diskResource, "pvc-hydra-" + id, volumeType)).ifPresent(
                coreVolumes::add
            );
        });
    ...
    public static CoreVolume buildSharedSharedVolume(
        ...
        if (resource != null && resource.getDisk() != null) {
            ...
            return new CoreVolume(volumeType, "/shared", "shared-dir", volumeOptions);
        }
