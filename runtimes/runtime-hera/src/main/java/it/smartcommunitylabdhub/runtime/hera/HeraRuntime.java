...
@Slf4j
public class HeraRuntime extends K8sWorkflowBaseRuntime<HeraWorkflowSpec, HeraRunSpec, HeraRunStatus, K8sRunnable> {
    ...
    @Override
    public HeraRunSpec build(@NotNull Workflow workflow, @NotNull Task task, @NotNull Run run) {
        //check run kind
        if (!isSupported(run)) {
