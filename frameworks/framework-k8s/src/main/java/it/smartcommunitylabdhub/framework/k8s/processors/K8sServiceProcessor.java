...
public class K8sServiceProcessor implements Processor<Run, K8sServiceStatus> {
            ...
            if (res != null && res.containsKey("service")) {
                try {
                    ...
                    if (res.containsKey("services")) {
                        ...
                        status.getService().setAliases(aliases);
                    }
    ...
    private K8sServiceStatus parseService(V1Service service) {
        if (
            service == null ||
        ...
        if (CoreServiceType.ClusterIP.name().equals(type) && !spec.getPorts().isEmpty()) {
            ...
            String url = String.format("%s.%s:%d", metadata.getName(), metadata.getNamespace(), port.getPort());
            builder.url(url);
