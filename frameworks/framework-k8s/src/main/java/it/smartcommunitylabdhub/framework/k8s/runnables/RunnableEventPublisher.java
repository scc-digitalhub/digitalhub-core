...
public class RunnableEventPublisher {
    ...
    public RunnableEventPublisher(MessageChannel channel) {
    ...
    public void publishEvent(RunnableChangedEvent<?> event) {
        ...
        log.debug("publish message for runnable {}", event.getRunnable().getId());
        if (log.isTraceEnabled()) {
            log.trace("publishing runnable event for runnable {}", event.getRunnable().getId());
        }
