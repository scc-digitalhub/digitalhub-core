...
public class AuditMetadataBuilder implements Converter<BaseEntity, AuditMetadata> {
    ...
    public AuditMetadataBuilder(AttributeConverter<Map<String, Serializable>, byte[]> cborConverter) {
        this.converter = cborConverter;
    }

    @Override
    public AuditMetadata convert(@NonNull BaseEntity entity) {
        if (!(entity instanceof MetadataEntity)) {
            return null;
        }

        //read metadata map as-is
        Map<String, Serializable> meta = converter.convertToEntityAttribute(((MetadataEntity) entity).getMetadata());
